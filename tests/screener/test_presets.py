...
def test_debt_free_blue_chip_mock():
    ...
    data = {
        ...
        "debt_to_equity": [
            ...
            0.01,
            0.20,
        ],  # C fails Debt-to-Equity (> 0.05) and interest check
    ...
    df_all = pd.DataFrame(
        {
            ...
            "debt_to_equity": [
                ...
                0.3,
                0.4,
            ],  # A declining (0.8 -> 0.5), B increasing (0.3 -> 0.4)
...
def test_presets_integration_with_db():
    ...
    for preset_name in presets:
        ...
        assert (
            5 <= len(df_res) <= 50
        ), f"Preset '{preset_name}' yielded {len(df_res)} companies, which is outside [5, 50] range."
