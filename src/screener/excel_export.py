...
def style_preset_sheet(ws, df: pd.DataFrame, preset_name: str):
    ...
    for r_idx, row in enumerate(df.itertuples(index=False), start=2):
        ...
        row._asdict() if hasattr(row, "_asdict") else row._fields
        ...
        for col_name, val in row_vals.items():
            ...
            apply_preset_conditional_formatting(
                cell, col_name, val, is_fin, is_df, preset_name, row_vals
            )
...
def apply_preset_conditional_formatting(
    cell,
    col_name: str,
    val,
    is_fin: bool,
    ...
    if preset_name == "Quality Compounder":
        if col_name == "return_on_equity_pct":
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val >= 15.0 else RED_FILL,
                    font=GREEN_FONT if f_val >= 15.0 else RED_FONT,
                )
            ...
            if is_fin:
                ...
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val <= 0.5 else RED_FILL,
                    font=GREEN_FONT if f_val <= 0.5 else RED_FONT,
                )
        elif col_name == "free_cash_flow_cr":
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val >= 0.0 else RED_FILL,
                    font=GREEN_FONT if f_val >= 0.0 else RED_FONT,
                )
        elif col_name == "operating_profit_margin_pct":
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val >= 12.0 else RED_FILL,
                    font=GREEN_FONT if f_val >= 12.0 else RED_FONT,
                )
        elif col_name == "interest_coverage":
            if is_df:
                apply_cell_style(cell, fill=GREEN_FILL, font=GREEN_FONT)
                ...
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val >= 3.0 else RED_FILL,
                    font=GREEN_FONT if f_val >= 3.0 else RED_FONT,
                )
    ...
    # --- Preset 2: Value Pick ---
        ...
        if col_name == "pe_ratio":
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val <= 35.0 else RED_FILL,
                    font=GREEN_FONT if f_val <= 35.0 else RED_FONT,
                )
        elif col_name == "pb_ratio":
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val <= 5.0 else RED_FILL,
                    font=GREEN_FONT if f_val <= 5.0 else RED_FONT,
                )
            ...
            if is_fin:
                ...
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val <= 1.2 else RED_FILL,
                    font=GREEN_FONT if f_val <= 1.2 else RED_FONT,
                )
    ...
    # --- Preset 3: Growth Accelerator ---
        ...
        if col_name in ["revenue_cagr_5yr", "pat_cagr_5yr"]:
            ...
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val >= 10.0 else RED_FILL,
                    font=GREEN_FONT if f_val >= 10.0 else RED_FONT,
                )
    ...
    # --- Preset 4: Dividend Champion ---
        ...
        if col_name == "dividend_yield_pct":
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val >= 2.0 else RED_FILL,
                    font=GREEN_FONT if f_val >= 2.0 else RED_FONT,
                )
        elif col_name == "return_on_equity_pct":
            if f_val is not None:
                apply_cell_style(
    ...
    # --- Preset 5: Debt-Free Blue Chip ---
        ...
        if col_name == "market_cap_crore":
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val >= 20000.0 else RED_FILL,
                    font=GREEN_FONT if f_val >= 20000.0 else RED_FONT,
                )
        elif col_name == "return_on_equity_pct":
            if f_val is not None:
                apply_cell_style(
    ...
    # --- Preset 6: Turnaround Watch ---
        ...
        if col_name == "debt_to_equity":
            ...
            apply_cell_style(
                cell,
                fill=GREEN_FILL if declined else RED_FILL,
                font=GREEN_FONT if declined else RED_FONT,
            )
        elif col_name == "return_on_equity_pct":
            if f_val is not None:
                apply_cell_style(
            ...
            if f_val is not None:
                apply_cell_style(
                    cell,
                    fill=GREEN_FILL if f_val >= 100.0 else RED_FILL,
                    font=GREEN_FONT if f_val >= 100.0 else RED_FONT,
                )
...
def generate_screener_excel(
    ...
    for preset_name, df_filtered in presets_dict.items():
        ...
        if "composite_quality_score" in df_sheet.columns:
            df_sheet.sort_values(
                by="composite_quality_score", ascending=False, inplace=True
            )
