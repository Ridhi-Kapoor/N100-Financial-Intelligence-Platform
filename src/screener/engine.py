...
def load_config(config_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    if not path.exists():
        logger.warning(
            f"Configuration file not found at {path.resolve()}. Returning empty dictionary."
        )
        return {}
...
def add_composite_quality_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(
        "Column 'composite_quality_score' not found. Calculating dynamically..."
    )
...
def load_screener_data(
    db_path: Union[str, Path], year: Optional[Union[str, int]] = None
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        df_market_cap = pd.read_sql_query(
        ...
        # Merge tables
        merged = pd.merge(
            df_ratios, df_companies, left_on="company_id", right_on="id", how="left"
        )
        ...
        merged = pd.merge(
            merged, df_pl, on=["company_id", "year"], how="left", suffixes=("", "_pl")
        )
        ...
        df_pl_interest = pd.read_sql_query(
            "SELECT company_id, year, interest FROM profitandloss", conn
        )
        df_pl_interest["company_id"] = (
            df_pl_interest["company_id"].astype(str).str.strip()
        )
...
def apply_screener_filters(df: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
    ...
    if not filters:
        logger.info(
            "No active filters found in config. Returning sorted base DataFrame."
        )
    ...
    for idx, row in df_filtered.iterrows():
        ...
        for filter_name, thresholds in filters.items():
            ...
            if not col_name:
                logger.warning(
                    f"Filtering skipped: Metric name '{filter_name}' is not recognized."
                )
                continue

            if col_name not in df_filtered.columns:
                logger.warning(
                    f"Filtering skipped: Column '{col_name}' corresponding to '{filter_name}' is missing."
                )
                continue
