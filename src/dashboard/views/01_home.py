...
# Page Header
st.markdown(
    '<h1 class="gradient-header">⚡ Nifty 100 Executive Overview</h1>',
    unsafe_allow_html=True,
)
st.markdown(
    '<p class="sub-header">Comprehensive Financial Intelligence & Market Analytics Platform</p>',
    unsafe_allow_html=True,
)
...
# ---------------------------------------------------------
avg_roe = (
    df_ratios_year["return_on_equity_pct"].mean()
    if not df_ratios_year.empty and "return_on_equity_pct" in df_ratios_year.columns
    else None
)
med_pe = (
    df_val_year["pe_ratio"].median()
    if not df_val_year.empty and "pe_ratio" in df_val_year.columns
    else None
)
med_de = (
    df_ratios_year["debt_to_equity"].median()
    if not df_ratios_year.empty and "debt_to_equity" in df_ratios_year.columns
    else None
)

total_companies_count = (
    len(df_ratios_year["company_id"].unique())
    if not df_ratios_year.empty and "company_id" in df_ratios_year.columns
    else len(df_companies)
)
if total_companies_count == 0:
    total_companies_count = len(df_companies)

med_cagr = (
    df_ratios_year["revenue_cagr_5yr"].median()
    if not df_ratios_year.empty and "revenue_cagr_5yr" in df_ratios_year.columns
    else None
)
...
with col_left:
    ...
    if not df_companies.empty:
        if "broad_sector" in df_companies.columns:
            ...
            df_sec_merged = df_companies.merge(
                df_sectors[["company_id", "broad_sector"]],
                left_on="id",
                right_on="company_id",
                how="left",
            )
        ...
        df_sec_merged["broad_sector"] = df_sec_merged["broad_sector"].fillna(
            "Other Services"
        )
        ...
        sector_counts = (
            df_sec_year.groupby("broad_sector").size().reset_index(name="count")
        )
...
with col_right:
    ...
    if not df_ratios_year.empty and "composite_quality_score" in df_ratios_year.columns:
        ...
        df_top5 = (
            df_ratios_year.dropna(subset=["composite_quality_score"])
        ...
        if not df_top5.empty:
            ...
            df_top5["company_name"] = df_top5["company_name"].fillna(
                df_top5["company_id"]
            )
