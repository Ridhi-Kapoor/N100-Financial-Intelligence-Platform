...
st.markdown(
    '<h1 class="gradient-header">🏢 Company Profile & Deep-Dive</h1>',
...
with col_search2:
    text_search = (
        st.text_input("Or enter custom Ticker:", value="", help="Direct ticker query")
        .strip()
        .upper()
    )
...
about_desc = company_info.get(
    "about_company", "Comprehensive financial profile for Nifty 100 constituent."
)
...
with k1:
    st.metric(
        "ROE", f"{roe_val:.2f}%" if pd.notna(roe_val) else "N/A", "Return on Equity"
    )

with k2:
    st.metric(
        "ROCE", f"{roce_val:.2f}%" if pd.notna(roce_val) else "N/A", "Return on Capital"
    )

with k3:
    st.metric(
        "Net Profit Margin",
        f"{npm_val:.2f}%" if pd.notna(npm_val) else "N/A",
        "PAT Margin",
    )
...
with k4:
    st.metric(
        "Debt-to-Equity",
        f"{de_val:.2f}" if pd.notna(de_val) else "N/A",
        "Leverage Ratio",
    )
...
with k5:
    st.metric(
        "Revenue CAGR (5Y)",
        f"{cagr_val:.2f}%" if pd.notna(cagr_val) else "N/A",
        "5-Year Growth",
    )
...
if not df_pl_clean.empty and len(df_pl_clean) < 10:
    st.info(
        f"ℹ️ **Limited historical data available:** Displaying {len(df_pl_clean)} years of financial records for {comp_name} ({selected_ticker})."
    )
elif df_pl_clean.empty:
    st.warning(
        f"⚠️ **Limited historical data available:** No multi-year P&L records found for {comp_name} ({selected_ticker})."
    )

with c_left:
    st.subheader("📊 10-Year Revenue & Net Profit")
    if (
        not df_pl_clean.empty
        and "sales" in df_pl_clean.columns
        and "net_profit" in df_pl_clean.columns
    ):
        fig_rev = go.Figure()
        fig_rev.add_trace(
            go.Bar(
...
if not df_pl_clean.empty:
    ...
    if not df_bs.empty and "year" in df_bs.columns:
        ...
        pbt = (
            df_chart_10y["profit_before_tax"].fillna(0)
            if "profit_before_tax" in df_chart_10y.columns
            else df_chart_10y["net_profit"].fillna(0)
        )
        interest = (
            df_chart_10y["interest"].fillna(0)
            if "interest" in df_chart_10y.columns
            else 0
        )
        ebit = pbt + interest

        eq = (
            df_chart_10y["equity_capital"].fillna(0)
            if "equity_capital" in df_chart_10y.columns
            else 0
        )
        res = (
            df_chart_10y["reserves"].fillna(0)
            if "reserves" in df_chart_10y.columns
            else 0
        )
        debt = (
            df_chart_10y["borrowings"].fillna(0)
            if "borrowings" in df_chart_10y.columns
            else 0
        )
...
if not latest_val.empty:
    ...
    if pd.notna(pe_ratio_val) and pe_ratio_val > 45:
        cons.append(
            f"Stock is trading at a high valuation multiple (P/E {pe_ratio_val:.1f}x)"
        )
...
with t1:
    ...
    if not df_pl.empty:
        ...
        st.dataframe(
            df_pl[display_cols].sort_values("year", ascending=False),
...
with t2:
    ...
    if not df_bs.empty:
        ...
        st.dataframe(
            df_bs[display_cols].sort_values("year", ascending=False),
            use_container_width=True,
            hide_index=True,
        )
    else:
...
with t3:
    ...
    if not df_cf.empty:
        ...
        st.dataframe(
            df_cf[display_cols].sort_values("year", ascending=False),
            use_container_width=True,
            hide_index=True,
        )
    else:
...
with t4:
    ...
    with cr1:
        ...
        if not df_ratios.empty:
            ...
            st.dataframe(
                df_ratios[ratio_cols].sort_values("year", ascending=False),
                use_container_width=True,
                hide_index=True,
            )
        else:
    ...
    with cr2:
        ...
        if not df_val.empty:
            ...
            st.dataframe(
                df_val[val_cols].sort_values("year", ascending=False),
                use_container_width=True,
                hide_index=True,
            )
        else:
