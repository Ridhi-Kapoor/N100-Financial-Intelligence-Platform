...
if not df_pl_clean.empty and len(df_pl_clean) < 10:
    st.info(f"ℹ️ **Limited historical data available:** Displaying {len(df_pl_clean)} years of financial records for {comp_name} ({selected_ticker}).")
elif df_pl_clean.empty:
    st.warning(f"⚠️ **Limited historical data available:** No multi-year P&L records found for {comp_name} ({selected_ticker}).")
