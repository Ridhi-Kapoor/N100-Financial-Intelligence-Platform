...
# Header
st.markdown(
    '<h1 class="gradient-header">📊 Broad & Sub-Sector Analytics</h1>',
    unsafe_allow_html=True,
)
st.markdown(
    '<p class="sub-header">Interactive sector bubble visualization, ROE vs Revenue distribution, sub-sector coloring, and median KPI benchmark comparisons.</p>',
    unsafe_allow_html=True,
)
...
df_pl_latest = (
    df_pl[pd.to_numeric(df_pl["year"], errors="coerce") == 2024].drop_duplicates(
        "company_id"
    )
    if not df_pl.empty
    else pd.DataFrame()
...
df_mc_latest = (
    df_mc[pd.to_numeric(df_mc["year"], errors="coerce") == 2024].drop_duplicates(
        "company_id"
    )
    if not df_mc.empty
    else pd.DataFrame()
...
df_rat_latest = (
    df_rat.drop_duplicates("company_id").copy() if not df_rat.empty else pd.DataFrame()
)

# Clean company keys
df_sec_clean = df_sec.drop_duplicates("company_id")[
...
if not df_comp.empty:
    comp_sub = df_comp.drop_duplicates("id")[
...
if not df_pl_latest.empty:
    df_master = df_master.merge(
        df_pl_latest[["company_id", "sales", "net_profit"]], on="company_id", how="left"
    )

if not df_mc_latest.empty:
    df_master = df_master.merge(
        df_mc_latest[["company_id", "market_cap_crore", "pe_ratio"]],
        on="company_id",
        how="left",
    )
...
if (
    "return_on_equity_pct" in df_master.columns
    and "roe_percentage" in df_master.columns
):
    df_master["roe_final"] = df_master["return_on_equity_pct"].fillna(
        df_master["roe_percentage"]
    )
...
with c4:
    ...
    st.metric(
        "Total Market Cap", f"₹{tot_mcap:,.0f} Cr" if pd.notna(tot_mcap) else "N/A"
    )
...
st.caption(
    "Bubble Size = Market Capitalization (₹ Cr) | Hover to inspect company metrics"
)

df_bubble = df_filtered.dropna(subset=["sales", "roe_final"]).copy()
df_bubble["market_cap_size"] = (
    df_bubble["market_cap_crore"].fillna(5000).clip(lower=1000)
)
...
sector_medians_df = (
    df_master.groupby("broad_sector")
    .agg({v: "median" for v in kpi_cols.values() if v in df_master.columns})
    .reset_index()
)
...
fig_kpi = px.bar(
    ...
    labels={
        ...
        metric_to_chart: f"Median {metric_to_chart}",
    },
...
with st.expander(
    f"📋 View All Companies in {selected_sector} ({len(df_filtered)} records)"
):
    table_cols = [
        "company_id",
        "company_name",
    ...
    table_show = df_filtered[
        [c for c in table_cols if c in df_filtered.columns]
    ].rename(
        columns={
            "company_id": "Ticker",
    ...
    st.dataframe(
        table_show.style.format(precision=2, na_rep="N/A"),
