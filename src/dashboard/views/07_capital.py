...
# Header
st.markdown(
    '<h1 class="gradient-header">💰 Capital Allocation Map & Cash Flow Patterns</h1>',
    unsafe_allow_html=True,
)
st.markdown(
    '<p class="sub-header">Categorize all 92 Nifty 100 companies into the 8 cash flow allocation patterns based on CFO, CFI, and CFF dynamics.</p>',
    unsafe_allow_html=True,
)
...
if not df_comp.empty:
    comp_sub = df_comp.drop_duplicates("id")[["id", "company_name"]].rename(
...
if not df_pl_latest.empty:
    df_master = df_master.merge(
        df_pl_latest[["company_id", "net_profit"]], on="company_id", how="left"
    )

if not df_mc_latest.empty:
    df_master = df_master.merge(
        df_mc_latest[["company_id", "market_cap_crore"]], on="company_id", how="left"
    )
...
df_master["market_cap_crore"] = pd.to_numeric(
    df_master["market_cap_crore"], errors="coerce"
).fillna(5000)
df_master["market_cap_size"] = df_master["market_cap_crore"].clip(lower=1000)

# Format strings for hover
df_master["display_label"] = (
    df_master["company_id"] + "<br>" + df_master["company_name"]
)
...
with c1:
    st.metric(
        "Shareholder Returns",
        pattern_counts.get("Shareholder Returns", 0),
        "CFO (+), CFI (-), CFF (-)",
    )
with c2:
    st.metric(
        "Reinvestor", pattern_counts.get("Reinvestor", 0), "High Capex Reinvestment"
    )
with c3:
    st.metric(
        "Growth Funded by Debt",
        pattern_counts.get("Growth Funded by Debt", 0),
        "CFF (+), CFI (-)",
    )
with c4:
    st.metric(
        "Distress / Liquidating",
        pattern_counts.get("Distress Signal", 0)
        + pattern_counts.get("Liquidating Assets", 0),
    )
...
st.caption(
    "Rectangle size = Market Capitalization (₹ Cr) | Color = Capital Allocation Pattern"
)
...
fig_treemap = px.treemap(
    df_master,
    path=[
        px.Constant("All Nifty 100 Companies"),
        "allocation_pattern",
        "display_label",
    ],
...
if selected_pattern == "All Patterns":
    ...
    df_table_filtered = df_master[
        df_master["allocation_pattern"] == selected_pattern
    ].copy()

st.write(
    f"Displaying **{len(df_table_filtered)}** companies matching pattern: `{selected_pattern}`"
)
...
table_show = df_table_filtered[table_cols].rename(
    columns={
        "company_id": "Ticker",
