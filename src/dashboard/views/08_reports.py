...
- Displays all available annual report years for the selected company.
- Clickable BSE Annual Report PDF link for each available year.
- Fast URL validation checking; displays a red "Report Unavailable" badge if missing or returning 404.
- Clean card and table grid layouts.
- Data Export Center & Executive Factsheet Generator tabs.
"""
...
st.markdown(
    '<h1 class="gradient-header">📄 Annual Reports & Data Export Portal</h1>',
    unsafe_allow_html=True,
)
st.markdown(
    '<p class="sub-header">Search BSE annual reports by company, inspect document filing links, generate executive tear-sheets, and export datasets.</p>',
    unsafe_allow_html=True,
)
...
def load_documents_data() -> pd.DataFrame:
    """Load annual report documents dataset from data/processed/documents.csv."""
...
def is_url_available(url: str) -> bool:
    """Check if annual report PDF URL is reachable and structurally valid."""
    if (
        not url
        or pd.isna(url)
    ...
    try:
        resp = requests.head(
            url_str, headers=HTTP_HEADERS, timeout=2.5, allow_redirects=True
        )
...
with t1:
    ...
    if not df_documents.empty and "company_id" in df_documents.columns:
        comp_docs = df_documents[
            df_documents["company_id"] == target_ticker
    ...
    if comp_docs.empty:
        ...
        st.write(
            f"Found **{len(comp_docs)}** annual report records across filing years:"
        )
            ...
            for j in range(cols_per_row):
                if i + j < len(doc_rows):
                    ...
                    with grid_cols[j]:
                        ...
                        if is_valid:
                            st.markdown(
                                f'<a href="{raw_url}" target="_blank" style="text-decoration: none;"><button style="background-color: #059669; color: white; border: none; padding: 8px 14px; border-radius: 6px; font-weight: 600; cursor: pointer; width: 100%;">📄 Open Report (PDF)</button></a>',
                                unsafe_allow_html=True,
                            )
                        else:
                            st.markdown(
                                '<div style="background-color: #991b1b; color: #fecaca; padding: 8px 14px; border-radius: 6px; font-weight: 600; text-align: center; font-size: 0.9rem;">🚨 Report Unavailable</div>',
                                unsafe_allow_html=True,
                            )
        ...
        with st.expander("📋 View Summary Table of Annual Report Filings"):
            ...
            for _, r in comp_docs.iterrows():
                ...
                status_str = (
                    "Available" if is_url_available(u_val) else "Report Unavailable"
                )
                table_data.append(
                    {
                        "Year": yr_val,
...
with t2:
    ...
    if not df_companies.empty:
        sel_comp = st.selectbox(
            "Select Target Company for Tear-Sheet:",
            company_options,
            key="factsheet_comp",
        )
        ...
        st.markdown(
            f"**Broad Sector:** {comp_info.get('broad_sector', 'N/A')} | **Sub-Sector:** {comp_info.get('sub_sector', 'N/A')}"
        )
        ...
        with col_f1:
            ...
            if not df_comp_ratios.empty:
                ...
                st.write(
                    f"- **ROE %:** {latest_ratio.get('return_on_equity_pct', 'N/A')}%"
                )
                st.write(
                    f"- **OPM %:** {latest_ratio.get('operating_profit_margin_pct', 'N/A')}%"
                )
                st.write(
                    f"- **Debt-to-Equity:** {latest_ratio.get('debt_to_equity', 'N/A')}"
                )
                st.write(
                    f"- **Free Cash Flow:** ₹{latest_ratio.get('free_cash_flow_cr', 'N/A')} Cr"
                )
                st.write(
                    f"- **Quality Score:** {latest_ratio.get('composite_quality_score', 'N/A')}/100"
                )
        ...
        with col_f2:
            ...
            if not df_comp_pl.empty:
                cols = [
                    c
                    for c in ["year", "sales", "net_profit", "opm_percentage", "eps"]
                    if c in df_comp_pl.columns
                ]
