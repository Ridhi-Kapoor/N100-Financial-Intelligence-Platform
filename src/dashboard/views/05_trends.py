...
from src.dashboard.utils.db import get_companies, get_pl, get_ratios, get_valuation, get_cf

# Page Header
st.markdown('<h1 class="gradient-header">📈 Multi-Year Financial Trend Analysis</h1>', unsafe_allow_html=True)
st.markdown('<p class="sub-header">Analyze historical performance trajectories, overlay key financial metrics, and inspect Year-over-Year (YoY) growth rates.</p>', unsafe_allow_html=True)
...
with col_metrics:
    selected_metric_keys = st.multiselect(
        ...
        default=["Revenue (Sales - ₹ Cr)", "Net Profit (PAT - ₹ Cr)", "ROE % (Return on Equity)"],
        max_selections=3,
...
for df_tmp in [df_pl, df_ratios, df_val, df_cf]:
    if not df_tmp.empty and "year" in df_tmp.columns:
        valid_yrs = pd.to_numeric(df_tmp["year"], errors="coerce").dropna().astype(int).tolist()
        years.update(valid_yrs)

sorted_years = sorted([y for y in years if y >= 2011])
if not sorted_years:
    st.warning(f"⚠️ **Limited historical data available:** No financial records available for {selected_company_str.split(' - ')[1]} ({selected_ticker}).")
    st.stop()

if len(sorted_years) < 10:
    st.info(f"ℹ️ **Limited historical data available:** Displaying {len(sorted_years)} years of financial trends for {selected_company_str.split(' - ')[1]} ({selected_ticker}).")
...
for y in sorted_years:
    ...
    if not df_pl.empty and "year" in df_pl.columns:
        ...
        if not sub_pl.empty:
            for col in ["sales", "net_profit", "operating_profit", "operating_profit_margin_pct"]:
    ...
    if not df_ratios.empty and "year" in df_ratios.columns:
        ...
        if not sub_rat.empty:
            for col in ["return_on_equity_pct", "earnings_per_share", "free_cash_flow_cr", "debt_to_equity"]:
...
has_other_unit = any(METRIC_DEFINITIONS[m][2] in ["%", "₹", "x"] for m in selected_metric_keys)
...
if use_secondary_y:
    fig.update_yaxes(title_text="Percentage (%) / Ratios", secondary_y=True, gridcolor="rgba(0,0,0,0)")
...
df_table_show = df_table[display_cols].rename(columns={
    "sales": "Revenue (₹ Cr)",
